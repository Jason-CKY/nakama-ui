...
import { IAuthContext, IAuthProvider, TInternalConfig, TRefreshTokenExpiredEvent, TTokenData, TTokenResponse } from './Types';
...
export const AuthProvider = ({ authConfig, children }: IAuthProvider) => {
    ...
    const [refreshTokenExpire, setRefreshTokenExpire] = useLocalStorage<number>('ROCP_refreshTokenExpire', epochAtSecondsFromNow(2 * FALLBACK_EXPIRE_TIME));
    const [token, setToken] = useLocalStorage<string>('ROCP_token', '');
    const [tokenExpire, setTokenExpire] = useLocalStorage<number>('ROCP_tokenExpire', epochAtSecondsFromNow(FALLBACK_EXPIRE_TIME));
    ...
    return <AuthContext.Provider value={{ tokenData, token, idToken, login, logOut, error, loginInProgress }}>{children}</AuthContext.Provider>;
};
